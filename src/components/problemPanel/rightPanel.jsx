...
function RightPanel({ problem }) {
    ...
    const availableLanguages = problem.codeSnippets
        ?.filter(s => allowedLanguages.includes(s.langSlug))
        .map(s => ({
            value: s.langSlug,
            label: s.lang,
            monacoLang: s.langSlug === 'cpp' ? 'cpp' : s.langSlug === 'python3' ? 'python' : s.langSlug
        })) || [];
...
int main() {
    ...
    return (
        <div id="right-panel-container" className="w-full h-full bg-zinc-950 flex flex-col">
            {/* Header */}
